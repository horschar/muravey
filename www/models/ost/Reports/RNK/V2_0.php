...
class V2_0 extends RNK
{
    ...
    public function generate ()
    {
        ...
        $html .= '<div style="text-align: center; margin: 20px 0; font-weight: bold;">
                    СПИСОК ВОЕННОСЛУЖАЩИХ ПО КОНТРАКТУ, СТОЯЩИХ НА КОТЛОВОМ ДОВОЛЬСТВИИ';

        $unit_title = '<br>';
        if ($this->unit !== 0)
            $unit_title = '<div style="margin: 16px 0;">подразделение: ' . $STRUCT_DATA[$this->unit]['title'] . '</div>';

        $html .= $unit_title . 'на ' . $this->date['str']. ' г.
                 </div>';

        $html_table = '<table class = "kotel_table2_0"><tr>';

        foreach ($this->settings['periods'] as $period_key => $period_data)
            $html_table .= '<td style="text-align: center;">' . $period_data['title'] . '</td>';
        $html_table .= '</tr><tr><td colspan="3" style="text-align: center;">' . $this->date['str'] . '</td></tr>';
